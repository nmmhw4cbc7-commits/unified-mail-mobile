...
export default function MailDetailScreen() {
  ...
  const mail = useMemo(() => messages.find((item) => item.id === id), [id, messages]);
  ...
  return <ScreenContainer edges={["top", "left", "right", "bottom"]}>
    ...
    <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.senderRow}><View style={[styles.avatar, { backgroundColor: `${account.color}22` }]}><Text style={[styles.avatarText, { color: account.color }]}>{mail.senderName.split(" ").map((part) => part[0]).join("")}</Text></View><View style={styles.senderInfo}><Text style={[styles.senderName, { color: colors.foreground }]}>{mail.senderName}</Text><Text selectable style={[styles.email, { color: colors.muted }]}>{mail.senderEmail}</Text><Text selectable style={[styles.email, { color: colors.muted }]}>An: {mail.recipients.join(", ") || account.email}</Text></View><Text style={[styles.time, { color: colors.muted }]}>{mail.dateLabel}</Text></View>
      ...
      <Text selectable style={[styles.body, { color: colors.foreground }]}>{mail.body || mail.preview || "(Kein Nachrichtentext)"}</Text>
