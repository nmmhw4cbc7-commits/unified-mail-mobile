...
async function sendGmail(accessToken: string, from: string, to: string, subject: string, body: string) {
  ...
  if (!response.ok) throw new Error(`Gmail konnte die Nachricht nicht annehmen (${response.status})`);
  const result = await response.json() as { id?: string };
  return { providerMessageId: result.id ?? null, accepted: true as const };
}
...
async function sendOutlook(accessToken: string, from: string, to: string, subject: string, body: string) {
  ...
  if (!response.ok) throw new Error(`Microsoft Graph konnte die Nachricht nicht annehmen (${response.status})`);
  return { providerMessageId: null, accepted: true as const };
}
...
export async function sendMail(userId: number, accountId: number, input: { to: string; subject: string; body: string }) {
  ...
  if (account.provider === "gmail") return { sent: true as const, provider: "gmail" as const, ...(await sendGmail(accessToken, account.email, input.to, input.subject, input.body)) };
  if (account.provider === "outlook") return { sent: true as const, provider: "outlook" as const, ...(await sendOutlook(accessToken, account.email, input.to, input.subject, input.body)) };
  throw new Error("Dieser Anbieter unterstützt noch keinen Versand.");
}
