...
export const appRouter = router({
  ...
  mail: router({
    ...
    updateStatus: publicProcedure.input(z.object({ deviceId: z.string().min(16), messageId: z.number(), unread: z.boolean().optional(), starred: z.boolean().optional() })).mutation(async ({ input }) => { const user = await db.getUserByOpenId(`device:${input.deviceId}`); if (!user) throw new Error("Device identity could not be found"); return db.updateMailMessageStatus(user.id, input.messageId, { unread: input.unread, starred: input.starred }); }),
